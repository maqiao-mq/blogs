...
每个cpu有一个folio_batch，应该是第一级缓存

每个node的pglist_data里面，有一个lruvec

每个memcg，会记录与每个node的关联情况，然后每个node记录一个lruvec


加入时机：

在处理page fault的上下文，即 `handle_mm_fault()`中，会处理各种类型的page fault。在各种page fault的处理中，如果page符合条件，就会调用 `folio_add_lru_vma()`或者 `lru_add_drain()`将page加入到percpu的folio_batch中进行管理。
