...
剩下的一个事情是，哪些page可以被swap，他们是什么时候被判定成可以被swap的。

内核把一个page写上去的时机，是在lru链表淘汰一个page的时候做的。

一个page，如果被 `folio_set_swapbacked()`，还是个匿名页，那么它在被淘汰的时候，就可能会被写到swap分区上。如果只是单纯地被设置了swapbacked，那可能表明它只是背后对应了一个文件，这个文件也可能是个普通文件。

在内核的缺页异常，处理匿名页映射的逻辑，即 `do_anonymous_page()`，内核会在调用 `folio_add_new_anon_rmap()`设置反向内存映射的时候，顺便调用 `folio_set_swapbacked()`标记它可以被换出到swap分区。

在内核调用 `shrink_folio_list`，一路往下选择了一个还在被map到某些进程空间内的page，准备要淘汰它时，就会先调用 `try_to_unmap_one`来准备先改掉pte，取消掉映射。这时，如果发现这个page是匿名页，就会调用 `folio_set_swapbacked()`来标记它可以被换出到swap分区，以便后续淘汰它的时候把它换进去。
